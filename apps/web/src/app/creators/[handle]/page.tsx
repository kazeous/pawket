...
import { PublicTip } from "../../../ui/tips/public-tip";
...
export default async function PublicCreatorPage({ params }: Readonly<{ params: Promise<{ handle: string }> }>) {
  ...
  return <AppShell context={`@${page.canonicalHandle}`} action={{ href: "/creators", label: "Khám phá" }}><article className="creator-publication stack">
    <header className="creator-publication__header stack">{page.cover ? <img className="creator-cover" src={`/media/${page.cover.assetId}/display`} width={page.cover.display.width} height={page.cover.display.height} alt="" /> : null}<div>{page.avatar ? <img className="creator-avatar" src={`/media/${page.avatar.assetId}/display`} width={page.avatar.display.width} height={page.avatar.display.height} alt={`Ảnh đại diện của ${page.displayName}`} /> : null}<p className="eyebrow">@{page.canonicalHandle}</p><h1>{page.displayName}</h1><p className="lede">{page.introduction}</p><p className="muted">{[page.primaryDiscipline, ...page.secondaryDisciplines].join(" · ")}</p></div><ReportForm label="Báo cáo trang này" target={{ targetType: "page", targetId: page.pageId, publicationRevisionId: page.revisionId }} /></header>
    ...
    <PublicTip handle={page.canonicalHandle} />
  </article></AppShell>;
}
