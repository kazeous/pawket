...
export function OwnerWorkbench() {
  ...
  function decide(event: FormEvent<HTMLFormElement>) {
    ...
    const action = async () => { await api(`/api/v1/admin/creator-applications/${detail.application.id}/decision`, post({ revisionId: detail.application.currentRevisionId, action: values.get("action"), reasonCode: values.get("reasonCode"), applicantExplanation: values.get("applicantExplanation"), ...(values.get("privateNote") ? { privateNote: values.get("privateNote") } : {}) }, { "idempotency-key": idempotencyKey, "if-match": String(detail.application.version) })); setDetail(null); setNotice({ tone: "success", text: "Đã ghi nhận quyết định và audit event." }); await load(); };
    return run(action);
  }

  function issueChallenge() {
    if (!detail?.payment.receivingAccountVersionId) return; const idempotencyKey = crypto.randomUUID();
    const action = async () => { await api(`/api/v1/admin/creator-applications/${detail.application.id}/deposit/challenge`, post({ revisionId: detail.application.currentRevisionId, accountVersionId: detail.payment.receivingAccountVersionId }, { "idempotency-key": idempotencyKey })); setNotice({ tone: "success", text: "Đã phát hành thử thách chuyển khoản 72 giờ." }); };
    return run(action);
  }
