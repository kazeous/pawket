...
export function CreatorTipConfirmation({ tip, onClose, onConfirmed }: Readonly<{ tip: CreatorTipProjection; onClose(): void; onConfirmed(): void }>) {
  const id = useId(); const busy = useRef(false); const firstInvalid = useRef<HTMLInputElement>(null); const totpInput = useRef<HTMLInputElement>(null);
  ...
  const [totpRequired, setTotpRequired] = useState(false); const [totp, setTotp] = useState(""); const [totpInvalid, setTotpInvalid] = useState(false); const [authRequired, setAuthRequired] = useState(false);
  ...
  async function confirm(event: FormEvent) {
    ...
    if (totpRequired && !/^\d{6}$/u.test(totp)) { setTotpInvalid(true); setError("Nhập mã xác thực gồm 6 chữ số."); totpInput.current?.focus(); return; }
  ...
  return <AlertDialog open onOpenChange={(open) => { if (!open && !busy.current) onClose(); }}>
    <AlertDialogContent data-tip-surface className="min-w-0 overflow-x-hidden">
      ...
      <form id={`${id}-form`} method="post" action={`/api/v1/creator/tips/${tip.id}/confirm`} onSubmit={(event) => void confirm(event)}>
        <FieldGroup>
          <Field data-invalid={invalid || undefined} data-disabled={locked || undefined}><FieldLabel htmlFor={`${id}-amount`}>Số tiền thực nhận (VND)</FieldLabel><Input ref={firstInvalid} id={`${id}-amount`} inputMode="numeric" autoComplete="off" maxLength={12} value={amount} disabled={locked} onChange={(e) => setAmount(e.target.value)} aria-invalid={invalid} aria-describedby={`${id}-evidence-help`} /></Field>
          <Field data-invalid={invalid || undefined} data-disabled={locked || undefined}><FieldLabel htmlFor={`${id}-reference`}>Nội dung trên giao dịch ngân hàng</FieldLabel><Input id={`${id}-reference`} autoComplete="off" spellCheck={false} maxLength={40} value={reference} disabled={locked} onChange={(e) => setReference(e.target.value)} aria-invalid={invalid} aria-describedby={`${id}-evidence-help`} /></Field>
          <Field data-invalid={invalid || undefined} data-disabled={locked || undefined}><FieldLabel htmlFor={`${id}-bank`}>Mã giao dịch ngân hàng</FieldLabel><Input id={`${id}-bank`} autoComplete="off" spellCheck={false} maxLength={100} value={bankId} disabled={locked} onChange={(e) => setBankId(e.target.value)} aria-invalid={invalid} aria-describedby={`${id}-evidence-help`} />
